...
export async function upsertUser(fid: number, username: string) {
  return supabase.from('users').upsert({ fid, username }, { onConflict: ['fid'] });
}
...
export async function checkAndResetTickets(fid: number) {
  ...
  if (data?.last_played !== today) {
    await supabase.from('users').update({ tickets: 5, last_played: today }).eq('fid', fid);
  }
}
...
export async function consumeTicket(fid: number): Promise<boolean> {
  ...
  await supabase.from('users').update({ tickets: data.tickets - 1 }).eq('fid', fid);
  return true;
}
